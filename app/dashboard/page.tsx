...
export default function DashboardPage() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-4 border-[#22c55e] border-t-transparent" /></div>;
        ...
        <div className="flex gap-1 overflow-x-auto px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {sidebarItems.map((item) => (
            <button
              ...
              className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium transition ${tab === item.id ? "bg-[#22c55e] text-white" : "bg-gray-100 text-blue-900 hover:bg-gray-200"}`}
            >
              {item.icon}
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-8">
        ...
        <aside className="hidden w-64 shrink-0 md:block">
          <div className="sticky top-24 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="h-24 bg-linear-to-br from-[#22c55e] to-[#16a34a]" />
            <div className="px-5 pb-5">
              <div className="-mt-10 mb-3 flex justify-center">
                <div className="flex h-20 w-20 items-center justify-center rounded-full border-4 border-white bg-[#22c55e] text-2xl font-bold text-white shadow-md">
                  {profile?.name.charAt(0).toUpperCase()}
                </div>
              </div>
              <div className="mb-4 text-center">
                <p className="text-base font-bold text-blue-900">{profile?.name}</p>
                <p className="mt-0.5 truncate text-xs text-blue-900">{profile?.email}</p>
                <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-[#eef7f1] px-3 py-1 text-xs font-medium text-[#22c55e]"><Shield size={11} />{pick(locale, { mn: "Хэрэглэгч", en: "User", ko: "사용자" })}</span>
              </div>
              {joinDate && <div className="mb-4 flex items-center justify-center gap-1.5 text-xs text-blue-900"><CalendarDays size={12} />{pick(locale, { mn: `${joinDate}-аас`, en: `Joined ${joinDate}`, ko: `${joinDate} 가입` })}</div>}
              <nav className="space-y-1">
                {sidebarItems.map((item) => (
                  <button key={item.id} onClick={() => { setTab(item.id); if (item.id === "notifications") loadNotifs(); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition ${tab === item.id ? "bg-[#22c55e] text-white shadow-sm" : "text-blue-900 hover:bg-gray-50"}`}>
                    {item.icon}
        ...
        <main className="min-w-0 flex-1">
          ...
          {tab === "cv" && (
            <div className="space-y-4">
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div className="h-16 bg-linear-to-r from-[#22c55e] to-[#16a34a]" />
                <div className="flex flex-col gap-4 px-6 pb-6 sm:flex-row sm:items-end">
                  <div className="-mt-8 flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border-4 border-white bg-[#22c55e] text-xl font-bold text-white shadow">{profile?.name.charAt(0).toUpperCase()}</div>
                  <div className="flex-1 pb-1">
                    <h2 className="text-lg font-bold text-blue-900">{profile?.name}</h2>
                    <div className="mt-1 flex flex-wrap gap-3 text-sm text-blue-900">
                      <span className="flex items-center gap-1.5"><Mail size={13} className="text-blue-900" />{profile?.email}</span>
                      {profile?.phone && <span className="flex items-center gap-1.5"><Phone size={13} className="text-blue-900" />{profile.phone}</span>}
                    </div>
                  </div>
                  <Link href="/jobs" className="inline-flex items-center gap-1.5 rounded-xl bg-[#22c55e] px-4 py-2 text-sm font-semibold text-white hover:bg-[#16a34a]"><Briefcase size={14} />{pick(locale, { mn: "Ажлын зар харах", en: "Browse Jobs", ko: "공고 보기" })}</Link>
                </div>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <h3 className="mb-5 text-sm font-bold uppercase tracking-wide text-blue-900">{pick(locale, { mn: "Мэдээлэл засах", en: "Edit Profile", ko: "프로필 수정" })}</h3>
                <form onSubmit={saveProfile} className="space-y-4">
                  {[{ label: pick(locale, { mn: "Бүтэн нэр", en: "Full Name", ko: "이름" }), value: cvForm.name, key: "name" as const }, { label: pick(locale, { mn: "Утасны дугаар", en: "Phone", ko: "전화번호" }), value: cvForm.phone, key: "phone" as const }].map(({ label, value, key }) => (
                    <div key={key}>
                      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-blue-900">{label}</label>
                      <input value={value} onChange={(e) => setCvForm({ ...cvForm, [key]: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#22c55e]" />
                    </div>
                  ))}
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-blue-900">{pick(locale, { mn: "И-мэйл", en: "Email", ko: "이메일" })}</label>
                    <input value={profile?.email || ""} disabled className="w-full rounded-xl border border-gray-100 bg-gray-50 px-4 py-2.5 text-sm text-blue-900 outline-none" />
                  </div>
                  {cvMsg && <p className="text-sm text-green-600">{cvMsg}</p>}
                  <button type="submit" disabled={cvSaving} className="w-full rounded-xl bg-[#22c55e] py-3 text-sm font-semibold text-white hover:bg-[#16a34a] disabled:opacity-60">{cvSaving ? pick(locale, { mn: "Хадгалж байна...", en: "Saving...", ko: "저장 중..." }) : pick(locale, { mn: "Хадгалах", en: "Save Changes", ko: "저장" })}</button>
                </form>
              </div>
            </div>
          )}
          ...
          {tab === "mycv" && (
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-2"><FileText size={16} className="text-[#22c55e]" /><h3 className="font-bold text-blue-900">{pick(locale, { mn: "Миний CV", en: "My CV", ko: "내 CV" })}</h3></div>
              ...
              <form onSubmit={saveStructuredCV} className="space-y-5">
                <div className="grid gap-4 sm:grid-cols-2">
                  {([
                    ...
                    <div key={key}>
                      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-blue-900">{label}</label>
                      <input required={required} value={cvFormData[key]} onChange={(e) => setCvFormData({ ...cvFormData, [key]: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#22c55e]" />
                    </div>
                  ))}
                  <div>
                ...
                {([
                  ...
                  <div key={key}>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-blue-900">{label}</label>
                    <textarea rows={rows} value={cvFormData[key]} onChange={(e) => setCvFormData({ ...cvFormData, [key]: e.target.value })} className="w-full resize-y rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm leading-relaxed outline-none focus:border-[#22c55e] focus:bg-white" />
                  </div>
                ))}
                ...
                <button type="submit" disabled={cvStructuredSaving} className="w-full rounded-xl bg-[#22c55e] py-3 text-sm font-semibold text-white hover:bg-[#16a34a] disabled:opacity-60">{cvStructuredSaving ? pick(locale, { mn: "Хадгалж байна...", en: "Saving...", ko: "저장 중..." }) : pick(locale, { mn: "CV хадгалах", en: "Save CV", ko: "CV 저장" })}</button>
              </form>
            </div>
          )}
          ...
          {tab === "saved" && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="font-bold text-blue-900">{pick(locale, { mn: "Хадгалсан зарууд", en: "Saved Jobs", ko: "저장한 공고" })}</h3>
                <span className="text-xs text-blue-900">{savedJobs.length} {pick(locale, { mn: "зар", en: "jobs", ko: "개" })}</span>
              </div>
              {savedLoading ? (
                <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-gray-100" />)}</div>
              ) : savedJobs.length === 0 ? (
                <div className="flex flex-col items-center gap-4 py-16 text-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#eef7f1]"><Bookmark className="h-8 w-8 text-[#22c55e]" /></div>
                  <p className="font-semibold text-blue-900">{pick(locale, { mn: "Хадгалсан зар байхгүй", en: "No saved jobs yet", ko: "저장한 공고가 없습니다" })}</p>
                  <Link href="/jobs" className="inline-flex items-center gap-2 rounded-xl bg-[#22c55e] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#16a34a]"><Briefcase size={14} />{pick(locale, { mn: "Зар хайх", en: "Browse Jobs", ko: "공고 보기" })}</Link>
                </div>
              ) : (
                <div className="space-y-3">
                  {savedJobs.map((fav) => (
                    <div key={fav.id} className="flex flex-col gap-3 rounded-xl border border-gray-200 p-4 hover:border-[#22c55e] sm:flex-row sm:items-center">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#dcfce7] font-bold text-[#22c55e]">{fav.job.company.name.charAt(0)}</div>
                        <div className="min-w-0 flex-1">
                          <Link href={`/jobs/${fav.job.id}`} className="font-semibold text-blue-900 hover:text-[#22c55e] truncate block">{fav.job.title}</Link>
                          <p className="text-xs text-blue-900 flex items-center gap-1"><MapPin size={11} />{fav.job.location} · {fav.job.company.name}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <Link href={`/jobs/${fav.job.id}`} className="rounded-lg bg-[#22c55e] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#16a34a] inline-flex items-center gap-1">{pick(locale, { mn: "Харах", en: "View", ko: "보기" })} <ArrowRight size={11} /></Link>
                        <button onClick={() => unsaveJob(fav.job.id)} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-blue-900 hover:bg-gray-50">{pick(locale, { mn: "Хасах", en: "Remove", ko: "삭제" })}</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {tab === "notifications" && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2"><Bell size={16} className="text-[#22c55e]" /><h3 className="font-bold text-blue-900">{pick(locale, { mn: "Мэдэгдэл", en: "Notifications", ko: "알림" })}</h3>{unread > 0 && <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white">{unread}</span>}</div>
                {unread > 0 && <button onClick={markAllRead} className="text-xs text-[#22c55e] hover:underline">{pick(locale, { mn: "Бүгдийг уншсан болгох", en: "Mark all read", ko: "모두 읽음" })}</button>}
              </div>
              {notifsLoading ? (
                ...
                <div className="flex flex-col items-center gap-3 py-16 text-center">
                  ...
                  <p className="text-blue-900">{pick(locale, { mn: "Мэдэгдэл байхгүй байна", en: "No notifications yet", ko: "알림이 없습니다" })}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {notifs.map((n) => (
                    <div key={n.id} className={`rounded-xl border p-4 transition ${n.isRead ? "border-gray-100 bg-white" : "border-[#22c55e] bg-[#eef7f1]"}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <p className={`text-sm font-semibold ${n.isRead ? "text-blue-900" : "text-blue-900"}`}>{n.title}</p>
                          <p className="mt-0.5 text-xs text-blue-900">{n.message}</p>
                        </div>
                        <span className="shrink-0 text-xs text-blue-900 flex items-center gap-1"><Clock size={11} />{new Date(n.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {tab === "settings" && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <div className="mb-4 flex items-center gap-2"><Globe size={16} className="text-[#22c55e]" /><h3 className="font-bold text-blue-900">{pick(locale, { mn: "Хэл", en: "Language", ko: "언어" })}</h3></div>
                <div className="flex items-center gap-4"><LanguageSwitcher /><span className="text-sm text-blue-900">{SUPPORTED_LOCALES.find((l) => l.code === locale)?.label}</span></div>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                <div className="mb-5 flex items-center gap-2"><KeyRound size={16} className="text-[#22c55e]" /><h3 className="font-bold text-blue-900">{pick(locale, { mn: "Нууц үг солих", en: "Change Password", ko: "비밀번호 변경" })}</h3></div>
                <form onSubmit={changePassword} className="space-y-4">
                  {[{ label: pick(locale, { mn: "Одоогийн нууц үг", en: "Current Password", ko: "현재 비밀번호" }), key: "current" as const }, { label: pick(locale, { mn: "Шинэ нууц үг", en: "New Password", ko: "새 비밀번호" }), key: "next" as const }, { label: pick(locale, { mn: "Нууц үг давтах", en: "Confirm Password", ko: "비밀번호 확인" }), key: "confirm" as const }].map(({ label, key }) => (
                    <div key={key}><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-blue-900">{label}</label><input type="password" value={pwForm[key]} onChange={(e) => setPwForm({ ...pwForm, [key]: e.target.value })} required className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-[#22c55e]" /></div>
                  ))}
                  {pwErr && <p className="text-sm text-red-600">{pwErr}</p>}
                  {pwMsg && <p className="text-sm text-green-600">{pwMsg}</p>}
                  <button type="submit" disabled={pwSaving} className="w-full rounded-xl bg-[#22c55e] py-3 text-sm font-semibold text-white hover:bg-[#16a34a] disabled:opacity-60">{pwSaving ? pick(locale, { mn: "Солж байна...", en: "Updating...", ko: "변경 중..." }) : pick(locale, { mn: "Нууц үг солих", en: "Change Password", ko: "비밀번호 변경" })}</button>
                </form>
              </div>
            </div>
          )}
