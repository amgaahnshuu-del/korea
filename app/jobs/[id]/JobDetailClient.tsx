...
export default function JobDetailClient() {
  ...
  if (loading) return <div className="flex min-h-screen flex-col"><Navbar /><div className="flex flex-1 items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-4 border-blue-700 border-t-transparent" /></div></div>;
  if (!job) return <div className="flex min-h-screen flex-col"><Navbar /><div className="flex flex-1 flex-col items-center justify-center gap-4"><p className="text-gray-500">{t.jobNotFound}</p><Link href="/jobs" className="text-blue-600 underline">{t.backToJobs}</Link></div></div>;
            ...
            <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {job.benefits.split("\n").filter(Boolean).map((b, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm text-gray-700"><Check className="h-4 w-4 shrink-0 text-green-500" />{b}</div>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="w-full shrink-0 lg:w-72">
          <div className="sticky top-24 mb-4 rounded-2xl border border-gray-200 bg-white p-5">
            ...
            {applied ? (
              ...
              <div className="mb-3 flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm font-semibold text-gray-400"><Clock size={16} />{pick(locale, { mn: "Зар дуусгавар болсон", en: "This job has expired", ko: "만료된 공고" })}</div>
            ) : (
              <button onClick={openModal} className="mb-3 w-full rounded-xl bg-blue-700 py-3 text-sm font-semibold text-white transition hover:bg-blue-800">
                {pick(locale, { mn: "CV илгээх", en: "Send CV", ko: "CV 보내기" })} →
              </button>
            )}

            <button onClick={toggleSave} disabled={saveLoading} className={`mb-3 w-full rounded-xl border py-2.5 text-sm font-semibold transition disabled:opacity-60 ${saved ? "border-yellow-300 bg-yellow-50 text-yellow-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
            ...
            <div className="mt-4 space-y-3 border-t border-gray-100 pt-4 text-sm text-gray-600">
              <div className="flex items-center gap-2"><MapPin size={14} className="shrink-0 text-gray-400" /><span>{job.location}</span></div>
              <div className="flex items-center gap-2"><Briefcase size={14} className="shrink-0 text-gray-400" /><span>{getJobTypeLabel(locale, job.type)}</span></div>
              <div className="flex items-center gap-2"><Building2 size={14} className="shrink-0 text-gray-400" /><span>{getCategoryLabel(locale, job.category)}</span></div>
              <div className="flex items-center gap-2"><Eye size={14} className="shrink-0 text-gray-400" /><span>{job.views} {labels.views} · {job._count.applications} {labels.applicants}</span></div>
              {job.expiresAt && (
                <div className={`flex items-center gap-2 ${expired ? "text-red-500" : "text-gray-500"}`}>
            ...
            {(job.contactPhone || job.phoneNumber || job.recruiterName || job.kakaoId) && (
              <div className="mt-4 space-y-2 border-t border-gray-100 pt-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">{pick(locale, { mn: "Холбоо барих", en: "Contact", ko: "연락처" })}</p>
                {job.recruiterName && <p className="flex items-center gap-2 text-sm text-gray-700"><Users size={13} className="text-gray-400" />{job.recruiterName}</p>}
                {(job.phoneNumber || job.contactPhone) && (
                  <a href={`tel:${job.phoneNumber || job.contactPhone}`} className="flex items-center gap-2 text-sm text-blue-700 hover:underline">
            ...
            {/* Report button */}
            <button onClick={() => setShowReport(true)} className="mt-4 flex w-full items-center justify-center gap-1.5 text-xs text-gray-400 hover:text-red-500 transition">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5">
            ...
            <div className="mb-3 flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 font-bold text-blue-700">{job.company.name.charAt(0)}</div>
              <div><p className="text-sm font-semibold">{job.company.name}</p><p className="text-xs text-gray-400">{job.company.industry || getCategoryLabel(locale, job.category)}</p></div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="mb-1 text-lg font-bold text-gray-900">{pick(locale, { mn: "CV илгээх", en: "Send CV", ko: "CV 보내기" })}</h3>
            <p className="mb-4 text-sm text-gray-500">{job.title} · {job.company.name}</p>
            {cvLoading ? (
              <div className="flex h-40 items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-4 border-blue-700 border-t-transparent" /></div>
            ) : cvText ? (
              <textarea value={cvText} onChange={(e) => setCvText(e.target.value)} rows={10} className="w-full resize-y rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm leading-relaxed outline-none focus:border-blue-400 focus:bg-white" />
            ) : (
              <div className="flex h-40 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-gray-200 bg-gray-50 text-center">
                <p className="text-sm text-gray-400">{pick(locale, { mn: "CV бичигдээгүй байна", en: "No CV written yet", ko: "CV가 없습니다" })}</p>
                <a href="/dashboard" className="rounded-lg bg-blue-700 px-4 py-1.5 text-xs font-semibold text-white hover:bg-blue-800">{pick(locale, { mn: "CV бичих", en: "Write CV", ko: "CV 작성" })}</a>
              </div>
            )}
            <div className="mt-4 flex gap-3">
              ...
              <button onClick={handleApply} disabled={applying || !cvText} className="flex-1 rounded-xl bg-blue-700 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-800 disabled:opacity-60">{applying ? t.submitting : pick(locale, { mn: "Илгээх", en: "Send", ko: "보내기" })}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-bold text-gray-900 flex items-center gap-2"><AlertTriangle size={16} className="text-red-500" />{pick(locale, { mn: "Зар гомдол гаргах", en: "Report Job", ko: "공고 신고" })}</h3>
              <button onClick={() => setShowReport(false)}><X size={18} className="text-gray-400 hover:text-gray-600" /></button>
            </div>
            {reportDone ? (
              <div className="flex flex-col items-center gap-3 py-6"><CheckCircle2 size={40} className="text-green-500" /><p className="font-semibold text-gray-800">{pick(locale, { mn: "Гомдлыг хүлээн авлаа", en: "Report submitted", ko: "신고 접수됨" })}</p></div>
            ) : (
              <>
                <div className="mb-4">
                  <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-gray-500">{pick(locale, { mn: "Шалтгаан", en: "Reason", ko: "이유" })}</label>
                  <div className="space-y-2">
                    {REPORT_REASONS.map((r) => (
                      <label key={r.value} className="flex cursor-pointer items-center gap-2 text-sm">
                        <input type="radio" name="reason" value={r.value} checked={reportReason === r.value} onChange={() => setReportReason(r.value)} className="text-blue-600" />
                ...
                <div className="mb-4">
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-500">{pick(locale, { mn: "Нэмэлт тайлбар (заавал биш)", en: "Additional description (optional)", ko: "추가 설명 (선택)" })}</label>
                  <textarea value={reportDesc} onChange={(e) => setReportDesc(e.target.value)} rows={3} className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm outline-none focus:border-blue-400 focus:bg-white" />
                </div>
